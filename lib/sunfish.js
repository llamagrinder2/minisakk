...
// ...
    else if (args[0] === "go")
    {
        ...
        console.log("Sunfish.engine: 'go' command received. maxdepth:", maxdepth, "think time:", think); // HOZZÁADVA EZ A SOR
        
        const start = perf.now();

        console.log("Sunfish.engine: Starting search loop."); // HOZZÁADVA EZ A SOR
        function done()
        {
            output("bestmove " + (move_str || last_move_str || '(none)'));
            console.log("Sunfish.engine: Search finished. Best move:", (move_str || last_move_str || '(none)')); // HOZZÁADVA EZ A SOR
        }
        function next()
        {
            ...
            while (++batch <= 1000)
            {
                ...
                if (nextSearch.done) {
                    console.log("Sunfish.engine: Search generator done."); // HOZZÁADVA EZ A SOR
                    return done();
                }
                let [depth, gamma, score, move] = nextSearch.value;
                console.log("Sunfish.engine: Search iteration - Depth:", depth, "Gamma:", gamma, "Score:", score, "Move:", move); // HOZZÁADVA EZ A SOR
                ...
                if (move && (score >= gamma))
                {
                    ...
                    console.log("Sunfish.engine: Found potential best move:", move_str, "Score:", score); // HOZZÁADVA EZ A SOR
                }
                if ((move_str && !isFinite(think)) || (depth > maxdepth) || (perf.now() - start > think))
                {
                    console.log("Sunfish.engine: Time or depth limit reached. Current best move:", move_str); // HOZZÁADVA EZ A SOR
                    return done();
                }
            }
            nextTick(next);
...
if (isWebWorker) {
    onmessage = function(e) {
        if ("string" === typeof e.data) {
            console.log("Sunfish Web Worker received command: ", e.data); // Hagyjuk meg a debug logot
            sunfish.engine(e.data, (output) => {
                console.log("Sunfish.engine outputting: ", output); // És ezt is hagyjuk meg
                postMessage(output);
            });
        }
    };
